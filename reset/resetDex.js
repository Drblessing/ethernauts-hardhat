const {
  ethernauts: {
    mumbai: { ethernaut, dex },
  },
} = require('../constants/constants.json');

const resetDex = async () => {
  console.log('Resetting dex...');
  ...
  const tx = await ethernauts.createLevelInstance(dex);
  ...
  // Get dex contract
  const dexInstance = event.args.instance;
